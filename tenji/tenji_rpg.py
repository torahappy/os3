...
MAX_APP_TIME = 3600000
APP_START_TIME = time.perf_counter()

class MyState(TypedDict):
    started: bool
    process_rpg: subprocess.Popen | None
    process_server: subprocess.Popen | None
    to_check_startup_flag: list[bool]

def exit_func():
    to_terminate = [my_state['process_rpg'], my_state["process_server"]]
...
src_path = os.path.join(
            basepath, "..", "rpg-helper", "games", "default"
        )
...
while True:
    ...
    subprocess.run(["/bin/wmctrl", "-x", "-r", "easyrpg-player.EasyRPG Player", "-e", "0,0,0,1800,1350"])
    subprocess.run(["/bin/wmctrl", "-x", "-r", "zbar.zbar", "-e", "0,1800,0,300,300"])
    ...
    if not my_state['started']:
        ...
        my_state['process_rpg'] = subprocess.Popen(["/bin/easyrpg-player", "x", "x", "Window", "--no-pause-focus-lost"], cwd=src_path, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
        my_state['process_server'] = subprocess.Popen(["/bin/python3", "../../server.py"], cwd=src_path, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
    ...
    rpg_pids = [p[0] for p in pids if p[1].startswith('/bin/easyrpg-player')]
    server_pids = [p[0] for p in pids if p[1].endswith('../../server.py')]

    to_check = [rpg_pids, server_pids]
