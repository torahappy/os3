...
// ---------------------------------------------------------------------------
let currentQrState = "login";
const loginQueue = new SimpleQueue();
const dataInputQueue = new SimpleQueue();
let endSignal = false;
// ---------------------------------------------------------------------------
//  Helpers
// ---------------------------------------------------------------------------
 ...
 */
async function verifySignatureAsync(data, signatureB64, signingKey, purpose) {
 ...
 */
async function rpgReadVars(count, offset = 99) {
    // 1. Read save data from main process
    const saveData = window.easyrpgPlayer.FS.readFile(SAVE_PATH);
 ...
 */
async function rpgWriteVars(variables, offset = 199) {
    // 1. Read save data from main process
    const saveData = window.easyrpgPlayer.FS.readFile(SAVE_PATH);
    // 2. Copy to Web Worker
    await callLcfLib("write_file", { filename: SAVE_PATH, data: saveData });
    ...
    const updated = await callLcfLib("read_file", {
        filename: SAVE_PATH,
    });
 ...
 * e.g. 199 1 [ 10001 ]
 */
async function rpgWriteError(code) {
...
// ---------------------------------------------------------------------------
async function qrReader(signingKey) {
    while (!endSignal) {
...
async function processQrLogin(qrData, signingKey) {
    ...
    // Validate
    const valid = await verifySignatureAsync(String(userId), signatureB64, signingKey, "ikiteikou_os_v0.0002_aone_cards");
    if (!valid) {
...
async function processQrDataInput(qrData, signingKey) {
    ...
    const valid = await verifySignatureAsync(path, signatureB64, signingKey, "ikiteikou_os_v0.0002_data_input");
    if (!valid) {
        dataInputQueue.put_nowait({
            errorCode: 2,
...
// ---------------------------------------------------------------------------
async function progressionLoop(db, signingKey) {
    let userId = null;
            ...
            if (userId === null) {
                ...
                if (loginResult === null) {
                    ...
                    if (currentProgression === null || currentProgression === undefined) {
                        throw new Error("SQL data type error; perhaps Database structure is malformed");
                    }
                    // log the login
                    await db.logins.add({
                        user_id: userId,
                ...
                if (check.length > 0 && check[0] >= 10000) {
                    throw new Error(`Desync from RPG with Error Code (while not logged in): ${check[0]}`);
                }
            }
            else {
                // ===========================================
                ...
                if (cmd === 2) {
                    ...
                    await db.users
                        .where("user_id")
                        .equals(userId)
                        .modify({ current_progression: nextProg });
