...
espeak().then((emscripten_functions) => {
  ...
  function init_consts() {
    ...
    c["CLAUSE_OPTIONAL_SPACE_AFTER"] =
        0x00008000;                // don't need space after the punctuation
    c["CLAUSE_TYPE"] = 0x000F0000; // phrase type
    c["CLAUSE_PUNCTUATION_IN_WORD"] =
        0x00100000; // punctuation character can be inside a word (Armenian)
    c["CLAUSE_SPEAK_PUNCTUATION_NAME"] =
        0x00200000; // speak the name of the punctuation character
    ...
    c["CLAUSE_PARAGRAPH"] =
        70 | c["CLAUSE_INTONATION_FULL_STOP"] | c["CLAUSE_TYPE_SENTENCE"];
    c["CLAUSE_EOF"] = 40 | c["CLAUSE_INTONATION_FULL_STOP"] |
                      c["CLAUSE_TYPE_SENTENCE"] | c["CLAUSE_TYPE_EOF"];
    c["CLAUSE_VOICE"] =
        0 | c["CLAUSE_INTONATION_NONE"] | c["CLAUSE_TYPE_VOICE_CHANGE"];
    c["CLAUSE_PERIOD"] =
        40 | c["CLAUSE_INTONATION_FULL_STOP"] | c["CLAUSE_TYPE_SENTENCE"];
    c["CLAUSE_COMMA"] =
        20 | c["CLAUSE_INTONATION_COMMA"] | c["CLAUSE_TYPE_CLAUSE"];
    c["CLAUSE_SHORTCOMMA"] =
        4 | c["CLAUSE_INTONATION_COMMA"] | c["CLAUSE_TYPE_CLAUSE"];
    c["CLAUSE_SHORTFALL"] =
        4 | c["CLAUSE_INTONATION_FULL_STOP"] | c["CLAUSE_TYPE_CLAUSE"];
    c["CLAUSE_QUESTION"] =
        40 | c["CLAUSE_INTONATION_QUESTION"] | c["CLAUSE_TYPE_SENTENCE"];
    c["CLAUSE_EXCLAMATION"] =
        45 | c["CLAUSE_INTONATION_EXCLAMATION"] | c["CLAUSE_TYPE_SENTENCE"];
    c["CLAUSE_COLON"] =
        30 | c["CLAUSE_INTONATION_FULL_STOP"] | c["CLAUSE_TYPE_CLAUSE"];
    c["CLAUSE_SEMICOLON"] =
        30 | c["CLAUSE_INTONATION_COMMA"] | c["CLAUSE_TYPE_CLAUSE"];
  ...
  function getPhonemes_pre(s_in) {
    ...
    emscripten_functions.HEAPU32.set(new Uint32Array([ text ]),
                                     align_u32(p_text));
    let ret_js = [];
    while (true) {
      emscripten_functions.HEAPU32.set(new Uint32Array([ 0 ]), align_u32(term));
      let ret = _espeak_TextToPhonemesWithTerminator(p_text, 8, 2, term);
      ret_js.push([
        UTF8ToString(ret, 1000000),
        emscripten_functions.HEAPU32[align_u32(term)]
      ]);
      if (emscripten_functions.HEAPU32[align_u32(p_text)] === 0) {
        break;
      }
      _free(ret);
    }
  ...
  function getPhonemes(s_in) {
    ...
    for (let i = 0; i < pre.length; i++) {
      ...
      if (terminator_str === "," || terminator_str === ":" ||
          terminator_str === ";") {
  ...
  function runInference(s_in) {
    ...
    let phonemeIds = Array.from(phonemes).map(
        (x) => WORKER_DATA.inference_config.phoneme_id_map[x][0]);
